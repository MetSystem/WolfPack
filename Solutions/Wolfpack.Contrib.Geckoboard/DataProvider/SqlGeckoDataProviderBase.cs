                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        data.Add(new LineChartData
                                     {
                                         Value = Convert.ToDouble((object) reader["ResultCount"]),
                                         When = Convert.ToDateTime((object) reader["GeneratedOnUtc"])
                                     });
                    }
                }
            }
        ...
        public IEnumerable<LineChartData> GetLineChartDataForCheckRate(LineChartArgs args)
        {
            ...
            using (var cmd = GetLineChartDataForCheckRateCommand(args))
            {
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var minutes = Convert.ToInt32((object) reader["MinutesBucket"]);
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        data.Value = (reader["Avg"] == DBNull.Value)
                                        ? 0
                                        : Convert.ToDouble((object) reader["Avg"]);
                        data.Min = (reader["Min"] == DBNull.Value)
                                             ? 0
                                             : Convert.ToDouble((object) reader["Min"]);
                        data.Max = (reader["Max"] == DBNull.Value)
                                             ? 0
                                             : Convert.ToDouble((object) reader["Max"]);
                    }
                }
            }
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        data.Value = (reader["Last"] == DBNull.Value)
                                        ? 0
                                        : Convert.ToDouble((object) reader["Last"]);
                    }
                }
            }
        ...
        public ComparisonData GetComparisonDataForSiteCheck(ComparisonArgs args)
        {
            ...
            using (var cmd = GetComparisonDataForSiteCheckCommand(args))
            {
                using (var reader = cmd.ExecuteReader())
                {
                    ...
                    {
                        if (++row == 1)
                            data.Number = Convert.ToDouble((object) reader["ResultCount"]);
                        else
                            data.Comparison = Convert.ToDouble((object) reader["ResultCount"]);
                    }
                }
            }
